...
function generateWeeklyCalendar(jsonHours, holidays, days, bookings) {
  ...
  for (var i = 0; i < jsonHours.length; i++) {
    ...
    tdCalendar.appendChild(
      document.createTextNode(
        jsonHours[i].start_time + " - " + jsonHours[i].end_time
      )
    );
    ...
    for (var j = 0; j < 7; j++) {
      ...
      var reservationsForDayAndHour = bookings.filter((reservation) => {
        var reservationStart = new Date(reservation.start_date);
        var reservationEnd = new Date(reservation.end_date);
        return (
          reservationStart.getDate() === currentDay.getDate() &&
          reservationEnd.getDate() === currentDay.getDate() &&
          reservationStart.getHours() <= parseInt(jsonHours[i].end_time.split(":")[0]) &&
          reservationEnd.getHours() >= parseInt(jsonHours[i].start_time.split(":")[0])
        );
      });

      reservationsForDayAndHour.forEach((reservation) => {
...
function loadWeekSchedule() {
  fetch(
    "http://localhost/2EvReservasAulas/services/serviceHoliday/holidayService.php",
    {
      method: "GET",
    }
  )
    ...
    .then((holidays) => {
      fetch(
        ...
        .then((calendarData) => {
          fetch(
            "http://localhost/2EvReservasAulas/services/serviceBookings/bookingsService.php",
            {
              method: "GET",
            }
          )
...
function updateMonth() {
  ...
  var currentMonth = currentDate.toLocaleDateString("default", {
    month: "long",
  });
...
fetch(
  "http://localhost/2EvReservasAulas/services/serviceSpace/serviceSpace.php"
)
  .then((response) => {
    if (!response.ok) {
  ...
  .catch((error) => {
    ...
    spaceSelect.innerHTML =
      '<option value="">Ocurrió un error al obtener las aulas.</option>';
  });
